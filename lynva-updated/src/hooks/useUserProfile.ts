...
export function useUserProfile() {
  ...
  const setupUserProfile = useCallback(async () => {
    if (!user || !user.email_confirmed_at || hasAttemptedSetup || userProfile || createBusiness.isPending || createUserProfile.isPending) {
      return
    }
  ...
  // Reset setup attempt when user changes
  useEffect(() => {
  ...
  // Auto-setup user profile when needed - ONLY for verified users
  useEffect(() => {
    if (user && user.email_confirmed_at && !isLoading && !userProfile && !hasAttemptedSetup) {
      console.log('Setting up profile for verified user:', user.email)
      setupUserProfile()
    }
